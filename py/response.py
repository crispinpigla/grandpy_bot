...
class Response:
	...
	def traitement(self):
		...
		for caractere0 in parser0.list_filtred:
			self._liste_recherche.append(recherche.Recherche(caractere0))
		...
		for recherche1 in self.liste_recherche:
			self._prepare_to_send_to_front.append( [recherche1.caractere_recherche,recherche1.liste_resultats] )
		...
		for resultat_parse in self._prepare_to_send_to_front:
			...
			response_send_front_resultat_parse[1].append( rnd.choice(config.SALUTATION_RESPONSE) )
			response_send_front_resultat_parse[1].append( rnd.choice(config.PRESENTATION_RESPONSE).replace('{{i}}', str(len(resultat_parse[1]))) )
